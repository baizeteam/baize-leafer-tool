...
const findAllCodeNode = (ast: RootNode) => {
  ...
  while (list.length) {
    ...
    if (
      typeof node === 'object' &&
      'tag' in node &&
      /^(CodeDemo|code-demo)$/.test(node.tag)
    ) {
...
const getFileListValue = (node: ComponentNode) => {
  const fileListProp = findProp(node, 'fileList', true) as
    | DirectiveNode
    | undefined;
...
const createBindNode = (
  ...
  const arrNode = value.map((item) => {
    ...
    for (const key in item) {
      properties.push(
        objectProperty(stringLiteral(key), stringLiteral(item[key]))
      );
    }
...
function viteRenderCode(vue: typeof vuePlugin, options: Options = {}) {
      ...
      compiler: {
        compile(source, options) {
          ...
          if (
            ...
            /\/node_modules\//.test(currentPath) ||
            !/(CodeDemo|code-demo)/.test(source)
          ) {
          ...
          for (const item of codeNodeList) {
            ...
            const fileListCode = fileListArrValue.map((item) => {
              ...
              const filePath = item.replace(
                curAlias,
                _originalConfig.resolve.alias[curAlias]
              );
            ...
            const codeDataNode = createBindStrNode(
              'codeData',
              escapeHtml(fileCode)
            );
  ...
  return Object.assign(vueP, {
    config(config: any, ...args: unknown[]) {
